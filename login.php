...
class User
{
    function login($json)
    {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            return json_encode([
                ...
                "user_last_attempts" => $user["user_last_attempts"]
            ]);
        }
        return 0;
    }
    ...
    function updateFailedAttempts($json)
    {
        ...
        if ($checkStmt->rowCount() > 0) {
            ...
            $updateSql = "UPDATE tbl_user SET user_failed_attempts = :user_failed_attempts";

            // If user_failed_attempts is 1, update the user_last_attempts column
            if ($user_failed_attempts == 1) {
                $updateSql .= ", user_last_attempts = NOW()";
            }

            $updateSql .= " WHERE user_email = :user_email";
